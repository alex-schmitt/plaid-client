/*
 * The Plaid API
 *
 * The Plaid REST API. Please see https://plaid.com/docs/api for more details.
 *
 * The version of the OpenAPI document: 2020-09-14_1.343.0
 *
 * Generated by: https://openapi-generator.tech
 */

/// CreditFreddieMacAssetTransactionDetailVoe25 : Documentation not found in the MISMO model viewer and not provided by Freddie Mac.

#[derive(
    Clone, Debug, PartialEq, Default, Serialize, Deserialize, bincode::Encode, bincode::Decode,
)]
pub struct CreditFreddieMacAssetTransactionDetailVoe25 {
    /// A vendor created unique Identifier.
    #[serde(rename = "AssetTransactionUniqueIdentifier")]
    pub asset_transaction_unique_identifier: String,
    /// Asset Transaction Date.
    #[serde(rename = "AssetTransactionDate")]
    pub asset_transaction_date: String,
    /// Asset Transaction Post Date.
    #[serde(rename = "AssetTransactionPostDate")]
    pub asset_transaction_post_date: String,
    #[serde(rename = "AssetTransactionType")]
    pub asset_transaction_type: crate::models::AssetTransactionType,
    /// Populate with who did the transaction.
    #[serde(
        rename = "AssetTransactionPaidByName",
        deserialize_with = "Option::deserialize"
    )]
    pub asset_transaction_paid_by_name: Option<String>,
    /// Populate with for whom the transaction is done.
    #[serde(
        rename = "AssetTransactionPaidToName",
        deserialize_with = "Option::deserialize"
    )]
    pub asset_transaction_paid_to_name: Option<String>,
    /// FI Provided - examples are atm, cash, check, credit, debit, deposit, directDebit, directDeposit, dividend, fee, interest, other, payment, pointOfSale, repeatPayment, serviceCharge, transfer.
    #[serde(
        rename = "AssetTransactionTypeAdditionalDescription",
        deserialize_with = "Option::deserialize"
    )]
    pub asset_transaction_type_additional_description: Option<String>,
    #[serde(
        rename = "AssetTransactionCategoryType",
        deserialize_with = "Option::deserialize"
    )]
    pub asset_transaction_category_type: Option<crate::models::AssetTransactionCategoryType>,
    /// FI provided Transaction Identifier.
    #[serde(
        rename = "FinancialInstitutionTransactionIdentifier",
        deserialize_with = "Option::deserialize"
    )]
    pub financial_institution_transaction_identifier: Option<String>,
}

impl CreditFreddieMacAssetTransactionDetailVoe25 {
    /// Documentation not found in the MISMO model viewer and not provided by Freddie Mac.
    pub fn new(
        asset_transaction_unique_identifier: String,
        asset_transaction_date: String,
        asset_transaction_post_date: String,
        asset_transaction_type: crate::models::AssetTransactionType,
        asset_transaction_paid_by_name: Option<String>,
        asset_transaction_paid_to_name: Option<String>,
        asset_transaction_type_additional_description: Option<String>,
        asset_transaction_category_type: Option<crate::models::AssetTransactionCategoryType>,
        financial_institution_transaction_identifier: Option<String>,
    ) -> CreditFreddieMacAssetTransactionDetailVoe25 {
        CreditFreddieMacAssetTransactionDetailVoe25 {
            asset_transaction_unique_identifier,
            asset_transaction_date,
            asset_transaction_post_date,
            asset_transaction_type,
            asset_transaction_paid_by_name,
            asset_transaction_paid_to_name,
            asset_transaction_type_additional_description,
            asset_transaction_category_type,
            financial_institution_transaction_identifier,
        }
    }
}
