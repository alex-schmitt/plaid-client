/*
 * The Plaid API
 *
 * The Plaid REST API. Please see https://plaid.com/docs/api for more details.
 *
 * The version of the OpenAPI document: 2020-09-14_1.343.0
 *
 * Generated by: https://openapi-generator.tech
 */

/// CreditFreddieMacVerificationOfAssetResponseVoe25 : Documentation not found in the MISMO model viewer and not provided by Freddie Mac.

#[derive(
    Clone, Debug, PartialEq, Default, Serialize, Deserialize, bincode::Encode, bincode::Decode,
)]
pub struct CreditFreddieMacVerificationOfAssetResponseVoe25 {
    #[serde(rename = "ASSETS")]
    pub assets: crate::models::CreditFreddieMacAssetsVoe25,
}

impl CreditFreddieMacVerificationOfAssetResponseVoe25 {
    /// Documentation not found in the MISMO model viewer and not provided by Freddie Mac.
    pub fn new(
        assets: crate::models::CreditFreddieMacAssetsVoe25,
    ) -> CreditFreddieMacVerificationOfAssetResponseVoe25 {
        CreditFreddieMacVerificationOfAssetResponseVoe25 { assets }
    }
}
